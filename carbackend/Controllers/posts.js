...
router.post('/',[auth,
  [
    ...
    .not()
    .isEmpty(),
    check('carstatus','carstatus is required')
    .not()
    .isEmpty()
  ]
  ],async(req,res)=>{
...
router.post('/approve', [auth
   ...
   if(!errors.isEmpty()){
    ...
    const carstatus= req.body.carstatus;
    ...
    try {
       ...
       let profile = await Post.findOne({_id:id});
       if(profile){ 
           profile = await Post.findOneAndUpdate(
             {_id: id},
